...
- `course-service` uses an in-memory H2 database during tests, so a local PostgreSQL instance is no longer required for `contextLoads`.
- `review-service` still starts with its default MongoDB configuration during `contextLoads`; the current smoke test passes even if MongoDB is not running, but you may still see connection-refused messages in the logs.
...
- `PUT /api/courses/{id}` updates an existing course and now returns `404 Not Found` if the requested course does not exist.
- `GET /api/reviews/{id}` and `DELETE /api/reviews/{id}` now return `404 Not Found` for missing reviews instead of a generic server error.
- `GET /api/course-aggregate/{id}/with-details` now requires both `COURSE-READ` and `REVIEW-READ` roles because the endpoint always reads from both downstream services.
